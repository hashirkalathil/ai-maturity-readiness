  ...
  return Object.entries(answers)
    .map(([questionId, selectedScore]) => {
      ...
      return [
        ...
        `  Question: ${question.questionText || question.question_text}`,
...
export async function generateGeminiReport(params) {
  ...
  if (!genAI) {
    console.log('[geminiReport] GEMINI_API_KEY not configured, using fallback report')
    return fallbackReport
  }

  try {
    console.log('[geminiReport] Starting report generation for', params.industryLabel)
      ...
      properties: {
        ...
        keyStrengths: {
          ...
          items: {
            ...
            required: ["dimension", "score", "headline", "insight"]
          }
        },
        keyGaps: {
          type: "array",
          items: {
            ...
            required: ["dimension", "score", "headline", "risk", "isBlocker"]
          }
        },
        immediateOpportunities: {
          type: "array",
          items: {
            ...
            required: ["title", "description", "timeframe", "estimatedImpact", "prerequisite"]
          }
        },
        risksAndBlockers: {
          type: "array",
          items: {
            ...
            properties: {
              ...
              triggersDimension: { type: "string" }
            },
            required: ["severity", "risk", "mitigation"]
          }
        },
        roadmapRecommendation: { type: "string" },
        blockerRuleTriggered: { type: "boolean" },
        blockerDimension: { type: "string" },
        unevenMaturityWarning: { type: "boolean" },
        unevenMaturityNote: { type: "string" }
      },
      required: [
        "executiveSummary", "maturityNarrative", "keyStrengths", "keyGaps", 
        "immediateOpportunities", "risksAndBlockers", "roadmapRecommendation", 
        "blockerRuleTriggered", "unevenMaturityWarning"
      ]
    };

    console.log('[geminiReport] Calling generateContent with schema validation')
    ...
    console.log('[geminiReport] Raw response length:', rawContent.length)

    try {
      const parsed = JSON.parse(rawContent)
      console.log('[geminiReport] Report generated successfully')
      return parsed
    } catch (e) {
      try {
        console.log('[geminiReport] First JSON parse failed, trying with markdown fence stripping')
        const parsed = JSON.parse(stripMarkdownFences(rawContent))
        console.log('[geminiReport] Report generated successfully after stripping markdown')
        return parsed
      } catch (e2) {
        console.error('[geminiReport] JSON Parse Error. Raw content snippet:', rawContent.substring(0, 500) + '...')
        throw new Error('Gemini returned invalid JSON')
      }
    }
  } catch (error) {
    console.error('[geminiReport] Generation failed, returning fallback:', error.message)
    return fallbackReport
  }
}
